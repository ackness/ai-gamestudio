...
**执行环境：**
- 脚本在后端进程的权限范围内直接执行（无沙箱隔离）。
- 环境变量已清理（`safe_env`），但脚本仍可访问文件系统。
- 使用 Python `-I` 隔离模式，禁用用户 site-packages。

### 10.2 安全风险声明

⚠️ **重要安全提示：**

当前插件脚本执行模型**不提供沙箱隔离**。插件脚本可以：
- 读写后端进程权限范围内的任何文件
- 执行任意系统命令（受进程权限限制）
- 访问网络资源
...
**审计机制：**
- 所有脚本执行会记录到 `data/audit/` 审计日志。
- 日志包含：插件名、脚本路径、参数、执行结果、stdout/stderr。
- 可用于事后审计和调试。

**前端提示：**
- 用户界面会在加载插件时显示安全风险提示。
- 用户需明确确认后才能启用包含脚本能力的插件。
