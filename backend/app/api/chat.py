...
async def _stream_process_message(
    ...
    *,
    ...
    """Run process_message and stream results over WebSocket."""
    full_response = ""
            ...
            try:
                if msg_type == "message":
                    ...
                    if not content:
                        ...
                        continue
                    await _stream_process_message(websocket, session_id, content)

                elif msg_type == "init_game":
                    await _handle_init_game(websocket, session_id, data)

                elif msg_type == "form_submit":
                    await _handle_form_submit(websocket, session_id, data)

                elif msg_type == "character_edit":
                    await _handle_character_edit(websocket, session_id, data)

                elif msg_type == "scene_switch":
                    await _handle_scene_switch(websocket, session_id, data)

                elif msg_type == "confirm":
                    await _handle_confirm(websocket, session_id, data)

                elif msg_type == "block_response":
                    await _handle_block_response(websocket, session_id, data)

                elif msg_type == "force_trigger":
                    await _handle_force_trigger(websocket, session_id, data)
...
async def _handle_init_game(
    ...
    await websocket.send_json(phase_event)

    # Trigger DM to generate opening narration
    await _stream_process_message(websocket, session_id, init_prompt)
